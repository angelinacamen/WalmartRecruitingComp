...
## EDA ##
#########
plot_missing(wlmFeatures)
plot_missing(wlmtTest)

### Impute Missing Markdowns
features <- wlmFeatures %>%
...
fullTrain <- left_join(wlmtTrain, imputed_features, by=c("Store", "Date")) %>%
  select(-IsHoliday.y) %>%
  rename(IsHoliday=IsHoliday.x) %>%
  select(-MarkDown_Total)

fullTest <- left_join(wlmtTest, imputed_features, by=c("Store", "Date")) %>%
  select(-IsHoliday.y) %>%
  rename(IsHoliday=IsHoliday.x) %>%
  select(-MarkDown_Total)
